...
void franke_kleinjung_sieve(int32_t p, int32_t r, int32_t A, uint8_t* sieve_array) {
    ...
    // Container to hold all valid hits for selection
    std::vector<Point> found_hits;
    ...
    while (x < max_x) {
        ...
        if (x >= A && x <= A * J_max) {
            ...
            // Reconstruct original 2D space coordinates from flat 1D offset
            int32_t orig_i = i - A / 2;
            int32_t orig_j = x / A;
            found_hits.push_back({orig_i, orig_j});
        }

        if (i >= b1) x += step_a;
        if (i < b0)  x += step_c;
    }
    cout << "Total lattice points found: " << hits << " (Expected ~518-519)\n\n";

    // --- Step 5: Print 10 Random Hits ---
    if (!found_hits.empty()) {
        cout << "Printing up to 10 random hits:\n";
        
        // Seed and initialize standard Mersenne Twister engine
        std::random_device rd;
        std::mt19937 g(rd());
        std::shuffle(found_hits.begin(), found_hits.end(), g);

        size_t print_count = std::min(found_hits.size(), static_cast<size_t>(10));
        for (size_t k = 0; k < print_count; ++k) {
            cout << "Random Hit #" << k + 1 << ": (i = " << found_hits[k].i 
                 << ", j = " << found_hits[k].j << ")\n";
        }
    }
}
